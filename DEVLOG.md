...
- 保留侧栏主题旁语言按钮的同时，在手机聊天顶部提供同款纯图标入口，避免默认空聊天页无法访问语言切换。
- 修复普通文本节点切回英文时仍使用初始英文查反向词典的问题，现在静态文案和输入属性都能双向切换。
- 工作区人物卡、世界书和文风的勾选现在会自动保存，取消选择后立即同步故事配置与工作区副本，切换面板不会恢复旧选择。
- 素材库编辑器新增 AI 草稿生成：用户描述目标后，AI 仅生成对应 JSON 草稿，用户审核或修改后点击 Save 才会真正创建角色、世界书或文风。
- 增加 `deploy/nocturne-atlas.service` systemd 服务定义，支持开机启动与异常自动重启；代码更新后可手动 `systemctl restart nocturne-atlas` 应用变更。
