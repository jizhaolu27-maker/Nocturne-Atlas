...
function createApiRouter({
  ...
  async function handleLibraryGenerateRoute(req, res, segments) {
    ...
    const typeGuidance = type === "worldbooks"
      ? "For a worldbook, capture only durable setting information: long-term rules, stable background, confirmed/revealed facts, and the current condition of this world, organization, or location. Put current plot events, scene details, character arcs, and next steps in Story Map or character cards instead. Keep storyState as a short present-condition summary, not a timeline."
      : type === "characters"
        ? "For a character card, keep traits, relationships, arc state, and notes focused on a recurring character; do not invent unrelated plot events."
        : "For a style profile, describe reusable prose constraints rather than story facts or plot events.";
    const prompt = `Create one reusable ${type.slice(0, -1)} library entry from the user's description. ${typeGuidance} Return JSON only, matching this exact shape (use concise, retrieval-friendly values, no markdown): ${schemas[type]}\nUser description:\n${description}`;
    try {
  ...
  async function handleStoriesRoute(req, res, segments) {
    ...
    if (segments[3] === "story-state" && segments.length === 4) {
      if (!getStory(storyId)) {
        notFound(res);
        return true;
      }
      if (req.method === "GET") {
        sendJson(res, 200, getStoryStateView(storyId));
        return true;
      }
      if (req.method === "PUT") {
        sendJson(res, 200, saveStoryState(storyId, await parseBody(req)));
        return true;
      }
    }
    if (segments[3] === "memory" && segments[4] && segments.length === 5 && req.method === "DELETE") {
      if (!getStory(storyId)) {
        notFound(res);
        return true;
      }
      sendJson(res, 200, deleteMemoryRecord(storyId, segments[4]));
      return true;
    }
    if (segments[3] === "memory" && segments[4] === "compress" && segments.length === 5 && req.method === "POST") {
      const story = getStory(storyId);
      if (!story) { notFound(res); return true; }
      ...
      if (!provider || !apiKey) { sendJson(res, 400, { error: "Configure a usable provider before compressing memory" }); return true; }
      const body = await parseBody(req);
      const current = readJsonLines(getStoryMemoryFile(storyId));
      if (body.accept === true) {
        const records = Array.isArray(body.records) ? body.records : [];
        if (!records.length || records.length > 240 || records.some((item) => !item || typeof item !== "object" || !String(item.summary || "").trim())) {
          sendJson(res, 400, { error: "A reviewed non-empty memory records list is required" }); return true;
        }
        const now = new Date().toISOString();
        for (const item of current) appendJsonLine(getStoryMemoryArchiveFile(storyId), { ...item, archivedAt: now, archiveReason: "Memory compression replacement" });
        const normalized = records.map((item, index) => ({
          ...item,
          id: /^[A-Za-z0-9][A-Za-z0-9_.:-]*$/.test(String(item.id || "")) ? String(item.id) : safeId("memory"),
          summary: String(item.summary).trim().slice(0, 1200),
          kind: String(item.kind || "plot_checkpoint").trim().slice(0, 60),
          tier: ["long_term", "short_term"].includes(item.tier) ? item.tier : "short_term",
          updatedAt: now,
        }));
        writeJsonLines(getStoryMemoryFile(storyId), normalized);
        sendJson(res, 200, { ok: true, count: normalized.length }); return true;
      }
      try {
        const preview = await buildStoryPreview(storyId);
        const result = await callOpenAICompatible({
          baseUrl: provider.baseUrl, apiKey, model: story.model || provider.model,
          messages: [
            { role: "system", content: "Compress the active story memory into faithful compact JSON. Return only {records:[...]}. Merge obvious duplicate summaries, preserve important facts, relationship changes, chronology, and uncertainty. Do not invent facts, delete unresolved conflicts, or modify character cards, worldbooks, or Story Map. Each record must keep summary, kind (relationship_update/world_state/character_update/plot_checkpoint), tier (long_term/short_term), importance, stability, scope, subjectIds, objectIds, entities, tags, and sourceMessageRange when available." },
            { role: "user", content: JSON.stringify({ records: current, workspace: preview.workspace, storyState: preview.storyState }, null, 2) },
          ], temperature: 0.1, topP: 1, max_tokens: 5000, responseFormat: { type: "json_object" },
        });
        const draft = parseGeneratedJson(result.content);
        sendJson(res, 200, { records: Array.isArray(draft.records) ? draft.records.slice(0, 240) : [] });
      } catch (error) { sendJson(res, 502, { error: error.message || "Failed to compress memory" }); }
      return true;
    }
    if (req.method === "POST" && segments[3] === "workspace" && ["characters", "worldbooks"].includes(segments[4]) && segments[5] && segments[6] === "compress") {
      if (!segments[5] || segments[5] === "." || segments[5] === ".." || /[\\/:*?"<>|\x00-\x1f]/.test(segments[5])) {
        sendJson(res, 400, { error: "Invalid workspace asset id" });
        return true;
      }
      const story = getStory(storyId);
      const assetType = segments[4] === "characters" ? "character" : "worldbook";
      const asset = story && readJson(path.join(getStoryWorkspaceDir(storyId, segments[4]), `${segments[5]}.json`), null);
      if (!story || !asset) {
        notFound(res);
        return true;
      }
      ...
      const apiKey = provider ? decryptSecret(provider.encryptedApiKey) : "";
      if (!provider || !apiKey) {
        sendJson(res, 400, { error: `Configure a usable provider before compressing a ${assetType}` });
        return true;
      }
      const body = await parseBody(req);
      try {
        if (body.accept === true) {
          ...
          const { changeLog, runtimeSummary, ...clean } = asset;
          const nextAsset = { ...clean, ...summary, workspaceUpdatedAt: new Date().toISOString() };
          writeJson(path.join(getStoryWorkspaceDir(storyId, segments[4]), `${segments[5]}.json`), {
            ...nextAsset,
          });
          sendJson(res, 200, { ok: true });
          return true;
        ...
        const result = await callOpenAICompatible({
          ...
          messages: [
            { role: "system", content: assetType === "character"
              ? "Compress a character card into faithful, compact JSON. Preserve all canon facts, relationships, current arc, and important notes. Do not invent or alter facts, and do not move plot events into the card. Return only JSON with keys core, traits, relationships, arcState, notes."
              : "Compress a worldbook into faithful, compact JSON. Preserve durable canon rules, stable background, revealed facts, and the current condition of the world, organization, or location. Keep storyState as a short present-condition summary; do not put timeline events, scene-by-scene developments, character arcs, or future goals there. Do not invent, delete unresolved conflicts, or alter facts. Return only JSON with keys title, category, rules, content, revealedFacts, storyState." },
            { role: "user", content: JSON.stringify(assetType === "character"
              ? { core: asset.core, traits: asset.traits, relationships: asset.relationships, arcState: asset.arcState, notes: asset.notes }
              : { title: asset.title, category: asset.category, rules: asset.rules, content: asset.content, revealedFacts: asset.revealedFacts, storyState: asset.storyState }, null, 2) },
          ],
          temperature: 0.1,
          topP: 1,
          max_tokens: assetType === "worldbook" ? 2400 : 1200,
          responseFormat: { type: "json_object" },
        });
        sendJson(res, 200, { runtimeSummary: parseGeneratedJson(result.content) });
      } catch (error) {
        sendJson(res, 502, { error: error.message || `Failed to compress ${assetType}` });
      }
      return true;
    }
    ...
    if (req.method === "POST" && segments[3] === "chat" && segments[4] === "stop") {
      if (!getStory(storyId)) {
        notFound(res);
        return true;
      }
      sendJson(res, 200, { ok: abortPendingGeneration(storyId) });
      return true;
    }
    ...
    if (req.method === "POST" && segments[3] === "proposals" && segments[4] === "generate") {
      try {
        sendJson(res, 200, await generateManualProposals({ storyId }));
      } catch (error) {
        sendJson(res, error.message === "Story not found" ? 404 : 502, { error: error.message || "Failed to generate proposals" });
      }
      return true;
    }
  ...
  function getErrorStatus(message) {
    return /contains invalid filename characters| is invalid| is required$|^Duplicate |^Unknown |requires two different character ids|cannot parent itself|parent cycle detected/.test(message) ||
