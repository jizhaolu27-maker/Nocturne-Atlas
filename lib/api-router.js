    ...
    if (req.method === "POST" && segments[3] === "workspace" && ["characters", "worldbooks"].includes(segments[4]) && segments[5] && segments[6] === "compress") {
      ...
      try {
        if (body.accept === true) {
          ...
          const nextAsset = assetType === "worldbook"
            ? { ...clean, ...summary, workspaceUpdatedAt: new Date().toISOString() }
            : { ...clean, runtimeSummary: summary, workspaceUpdatedAt: new Date().toISOString() };
