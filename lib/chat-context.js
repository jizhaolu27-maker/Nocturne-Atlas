const { normalizeRuntimeMemoryState } = require("./memory-runtime");
...
function createChatContextTools({
  ...
  function buildWorkspace(storyId, enabled = {}) {
    ...
    return {
      ...
      injectionModes: enabled.injectionModes || { characters: {}, worldbooks: {}, styles: {} },
    };
  }

  function loadRuntimeMemoryState(storyId) {
    ...
    const normalized = normalizeRuntimeMemoryState({ memoryRecords, memoryChunks });
    if (normalized.recordsChanged && typeof writeJsonLines === "function") {
      writeJsonLines(getStoryMemoryFile(storyId), normalized.memoryRecords);
    }
    if (normalized.chunksChanged && typeof writeJsonLines === "function") {
      writeJsonLines(getStoryMemoryChunkFile(storyId), normalized.memoryChunks);
    }
    return normalized;
  }
  ...
  async function buildChatContext(storyId, body) {
    ...
    const storyState = getStoryStateView(storyId);
    const userMessage = {
      id: safeId("msg"),
      role: "user",
