function createChatTurnTools({
  ...
  async function finalizeChatTurn({
    ...
    const ensureActive = () => {
      if (signal?.aborted) {
        const error = new Error("Chat generation aborted");
    ...
    // Proposal generation is manual-only. Keep diagnostics explicit so a chat
    // turn cannot be mistaken for an automatic proposal check.
    ...
    const finalMemoryRecords = transientMemoryCandidate
      ? [...(memoryUpdate.records || memoryRecords), transientMemoryCandidate]
      : memoryUpdate.records || memoryRecords;
