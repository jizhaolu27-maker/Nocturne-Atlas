...
function createChatTools({
  ...
  function getPendingGeneration(storyId) {
    const pending = pendingGenerations.get(String(storyId || ""));
    if (!pending) {
      return null;
    }
    return {
  ...
  function abortPendingGeneration(storyId) {
    const pending = pendingGenerations.get(String(storyId || ""));
  ...
  async function handleChatStream(req, res, storyId, body) {
    ...
    const pendingId = safeId("generation");
    const startedAt = new Date().toISOString();
    pendingGenerations.set(storyId, {
      id: pendingId,
      storyId,
      userMessage: { ...chat.userMessage },
      assistantText: "",
    ...
    const updatePendingGeneration = (patch) => {
      const current = pendingGenerations.get(storyId);
    ...
    const clearPendingGeneration = () => {
      if (pendingGenerations.get(storyId)?.id === pendingId) {
        pendingGenerations.delete(storyId);
      }
    };
    ...
    try {
      ...
      while (true) {
        ...
        for (const rawLine of lines) {
          ...
          if (text) {
            assistantText += text;
            updatePendingGeneration({ assistantText });
    ...
    try {
      updatePendingGeneration({ status: "finalizing" });
      const groundedResult = await maybeRepairGrounding({
        story: chat.story,
        provider: chat.provider,
        apiKey: chat.apiKey,
      ...
      if (groundedResult.groundingRepair?.applied && groundedResult.assistantText !== assistantText) {
        updatePendingGeneration({ assistantText: groundedResult.assistantText });
        sendEvent({ type: "replace", text: groundedResult.assistantText });
      }
      const onStage = (stage) => {
        updatePendingGeneration({ status: stage });
        sendEvent({ type: "stage", stage });
      };
      const payload = await finalizeChatTurn({
        storyId,
        story: chat.story,
      ...
      if (abortController.signal.aborted || clientClosed || error?.name === "AbortError") {
        sendEvent({ type: "aborted" });
      } else {
  ...
  async function buildStoryPreview(storyId) {
    ...
    const runtimeMemoryState = loadRuntimeMemoryState(storyId);
    const memoryRecords = runtimeMemoryState.memoryRecords;
    const memoryChunks = runtimeMemoryState.memoryChunks;
    ...
    const currentContextInfo = await buildContextBlocks(story, messages, memoryRecords, workspace, {
      memoryChunks,
      storyState: getStoryStateView(storyId),
      embeddingOptions: {
    ...
    return {
      story,
      messages: readJsonLines(getStoryMessagesFile(storyId)),
      pendingGeneration: getPendingGeneration(storyId),
      memoryRecords,
