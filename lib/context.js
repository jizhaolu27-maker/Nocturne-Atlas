...
function createContextTools({
  ...
  function buildStoryMapRetrievalText(storyState = {}, workspace = {}, currentUserInput = "") {
    ...
    const selected = entries.sort((left, right) => right.score - left.score || Number(right.item.sortKey || right.item.order || 0) - Number(left.item.sortKey || left.item.order || 0)).slice(0, 6);
