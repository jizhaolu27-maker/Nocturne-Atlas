function createContextTools({
  ...
  function buildCharacterAnchorText(characters = []) {
    return characters
      .map((item) =>
        [
          `Character: ${item.name}`,
  ...
  function buildWorldbookAnchorText(worldbooks = []) {
    return worldbooks
      .map((item) =>
        [
          `World: ${item.title}`,
  ...
  async function buildContextBlocks(story, messages, memoryRecords, workspace, options = {}) {
    ...
    const knowledgeSelection =
      typeof retrieveKnowledgeChunks === "function"
        ? await retrieveKnowledgeChunks({
            ...
            userMessage: options.currentUserInput || "",
            embeddingOptions,
            maxItems: options.maxKnowledgeItems || 4,
          })
        : { selectedChunks: [], retrievalMeta: { chunkCount: 0, activeMode: "lexical" } };
    ...
    pushBlock("knowledge:retrieved", formatKnowledgeContext ? formatKnowledgeContext(knowledgeSelection.selectedChunks) : "", 92);
    ...
    pushBlock("characters", buildCharacterAnchorText(workspace.characters), 90);

    pushBlock("worldbook", buildWorldbookAnchorText(workspace.worldbooks), 88);
