const { extractSearchTerms, normalizeStringList, unique } = require("./text-utils");
...
function createContextTools({
  ...
  function normalizeQueryEmbeddingState(value = {}) {
  ...
  function buildCharacterAnchorText(characters = [], options = {}) {
    ...
    return characters
      .map((item) => {
        const source = item.runtimeSummary && typeof item.runtimeSummary === "object" ? { ...item, ...item.runtimeSummary } : item;
        return (
        compact
          ? [
              `Character: ${source.name || item.name}`,
              source.core?.role ? `Role: ${source.core.role}` : "",
              source.arcState?.current ? `Arc: ${source.arcState.current}` : "",
            ]
              .filter(Boolean)
              .join(" / ")
          : [
              `Character: ${source.name || item.name}`,
              `Role: ${source.core?.role || ""}`,
              source.core?.age != null ? `Age: ${source.core.age}` : "",
              source.core?.background ? `Background: ${source.core.background}` : "",
              `Traits: ${normalizeStringList(source.traits).join(", ")}`,
              Object.keys(source.relationships || {}).length
                ? `Relationships: ${Object.entries(source.relationships).map(([name, relation]) => `${name}=${relation}`).join(", ")}`
                : "",
              `Arc: ${source.arcState?.current || ""}`,
  ...
  function buildStoryDirectionText(storyState = {}) {
    const current = storyState?.currentOutline || null;
    const threads = Array.isArray(storyState?.activePlotThreads) ? storyState.activePlotThreads.slice(0, 5) : [];
    if (!current && !threads.length) {
      return "";
    }
    return [
      "Reviewed story direction:",
      current
        ? `Current outline node: ${current.type || "node"} / ${current.title || "Untitled"} / ${current.status || "active"}${current.storyTime ? ` / story time ${current.storyTime}` : ""}.`
        : "Current outline node: not set.",
      current?.summary ? `Direction summary: ${current.summary}` : "",
      ...threads.map((item, index) =>
        [
          `Plot thread ${index + 1}: ${item.title || item.id || "Untitled"} (${item.status || "active"}).`,
          item.currentGoal ? `Goal: ${item.currentGoal}.` : "",
          item.nextStep ? `Next intended step: ${item.nextStep}.` : "",
          item.stakes ? `Stakes: ${item.stakes}.` : "",
        ].filter(Boolean).join(" ")
      ),
      "Treat this block as reviewed narrative direction, not proof that planned events have already happened.",
    ].filter(Boolean).join("\n");
  }

  function buildStoryMapRetrievalText(storyState = {}, workspace = {}, currentUserInput = "") {
    const query = String(currentUserInput || "").trim();
    if (!query) return "";
    const terms = extractSearchTerms(query, { maxItems: 32 });
    if (!terms.length) return "";
    const characters = new Map((workspace.characters || []).map((item) => [item.id, item.name || item.id]));
    const fixedIds = new Set([
      storyState.currentOutline?.id,
      ...(storyState.activePlotThreads || []).map((item) => item.id),
    ].filter(Boolean));
    const entries = [];
    const add = (kind, item, fields, status = item.status || "planned") => {
      const haystack = [item.id, ...fields].filter(Boolean).join(" ").toLowerCase();
      const matches = terms.filter((term) => haystack.includes(String(term).toLowerCase()));
      if (!matches.length) return;
      entries.push({ kind, item, status, score: matches.length + (matches.some((term) => haystack.startsWith(String(term).toLowerCase())) ? 1 : 0) });
    };
    for (const item of storyState.outlineNodes || []) {
      add("outline", item, [item.title, item.summary, item.storyTime, item.locationId, ...(item.characterIds || [])]);
    }
    for (const item of storyState.plotThreads || []) {
      add("plot thread", item, [item.title, item.summary, item.currentGoal, item.nextStep, item.stakes, ...(item.characterIds || [])]);
    }
    for (const item of storyState.timelineEvents || []) {
      add("timeline event", item, [item.title, item.summary, item.storyTime, item.locationId, ...(item.characterIds || [])]);
    }
    for (const item of storyState.relationshipEvents || []) {
      const source = characters.get(item.sourceId) || item.sourceId;
      const target = characters.get(item.targetId) || item.targetId;
      add("relationship", item, [source, target, item.sourceId, item.targetId, item.type, item.label, item.storyTime, item.note]);
    }
    const selected = entries
      .filter(({ item }) => !fixedIds.has(item.id))
      .sort((left, right) => right.score - left.score || Number(right.item.sortKey || right.item.order || 0) - Number(left.item.sortKey || left.item.order || 0))
      .slice(0, 6);
    if (!selected.length) return "";
    return [
      "Relevant reviewed story map entries:",
      ...selected.map(({ kind, item, status }) => {
        const planned = ["planned", "superseded", "discarded"].includes(status);
        const label = planned ? `${status} / not established canon` : status;
        const people = kind === "relationship" ? ` ${characters.get(item.sourceId) || item.sourceId} ↔ ${characters.get(item.targetId) || item.targetId}.` : "";
        return `${kind}: ${item.title || item.label || item.type || item.id} (${label}).${people} ${item.summary || item.note || item.currentGoal || item.nextStep || ""}`.trim();
      }),
      "Use these map entries only when relevant to the current request; planned or discarded entries are not proof that events happened.",
    ].join("\n");
  }
  ...
  async function buildContextBlocks(story, messages, memoryRecords, workspace, options = {}) {
    ...
    const queryEmbeddingValue =
      ...
      (embeddingOptions.mode === "on" && typeof buildQueryEmbedding === "function"
        ...
        : null);
    const queryEmbeddingState = normalizeQueryEmbeddingState(queryEmbeddingValue);
    const memorySelection = selectRelevantMemoryRecords(memoryRecords, {
      ...
      embeddingOptions,
      queryEmbedding: queryEmbeddingState.vector,
      queryEmbeddingProvider: queryEmbeddingState.provider,
      queryEmbeddingModel: queryEmbeddingState.model,
      queryEmbeddingSignature: queryEmbeddingState.signature,
    ...
    const knowledgeWorkspace = {
      ...workspace,
      characters: (workspace.characters || []).filter((item) => workspace.injectionModes?.characters?.[item.id] !== "always"),
      worldbooks: (workspace.worldbooks || []).filter((item) => workspace.injectionModes?.worldbooks?.[item.id] !== "always"),
      // Writing style is a standing generation constraint, not a retrieval fact.
      styles: [],
    };
    ...
    const alwaysCharacters = (workspace.characters || []).filter(
      (item) => injectionModes.characters?.[item.id] === "always"
    );
    const alwaysWorldbooks = (workspace.worldbooks || []).filter(
      (item) => injectionModes.worldbooks?.[item.id] === "always"
    );
    const alwaysStyles = workspace.styles || [];

    const pushBlock = (label, content, priority, pinned = false) => {
      if (!content) {
        return;
      }
      blocks.push({
    ...
    pushBlock("story:direction", buildStoryDirectionText(options.storyState), 98, true);
    pushBlock("story:map_retrieved", buildStoryMapRetrievalText(options.storyState, workspace, options.currentUserInput), 91);
    pushBlock(
      "knowledge:retrieved",
    ...
    pushBlock(
      "style",
      buildStyleAnchorText(alwaysStyles.length ? alwaysStyles : focusedStyles, { compact: useKnowledgeRag }),
      useKnowledgeRag ? 76 : 82,
      alwaysStyles.length > 0
    );
    pushBlock(
      "characters",
      buildCharacterAnchorText(alwaysCharacters.length ? alwaysCharacters : focusedCharacters, { compact: alwaysCharacters.length ? false : useKnowledgeRag }),
      alwaysCharacters.length ? 99 : useKnowledgeRag ? 80 : 90,
      alwaysCharacters.length > 0
    );
    pushBlock(
      "worldbook",
      buildWorldbookAnchorText(alwaysWorldbooks.length ? alwaysWorldbooks : focusedWorldbooks, { compact: useKnowledgeRag }),
      useKnowledgeRag ? 78 : 88,
      alwaysWorldbooks.length > 0
    );
    ...
    for (const block of sorted) {
      ...
      if (!block.pinned && usedTokens + block.tokens > Math.floor(maxTokens * 0.82) && selected.length > 4) {
        continue;
      }
      selected.push(block);
  ...
  return {
    ...
    buildContextBlocks,
    buildStoryDirectionText,
    buildStoryMapRetrievalText,
    classifyPressure,
