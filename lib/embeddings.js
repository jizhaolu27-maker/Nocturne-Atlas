...
function buildEmbeddingSignature(provider = "", model = "") {
  const normalizedProvider = String(provider || "").trim().toLowerCase();
  if (!normalizedProvider) {
    return "";
  }
  const normalizedModel =
    normalizedProvider === "hash_v1" ? "hash_v1" : String(model || "").trim();
...
function createEmbeddingTools() {
  ...
  function extractFocusClauses(text) {
    return String(text || "")
      .replace(/\r\n/g, "\n")
      .split(/[\n.!?;:\u3002\uff01\uff1f\uff1b\uff1a]+/)
      .map((item) => item.trim())
  ...
  function buildQueryEmbeddingText({ userMessage = "", messages = [], workspace = {} }) {
    ...
    const recent = recentMessages
      ...
      .join("\n");
    const focusClauses = [
      ...extractFocusClauses(userMessage),
      ...recentMessages.flatMap((item) => extractFocusClauses(item.content)),
    ]
      .filter((item, index, list) => list.indexOf(item) === index)
      .slice(0, 6);
    const workspaceText = [
      ...(workspace.characters || []).flatMap((item) => [item.name, item.core?.role, ...(item.traits || []).slice(0, 3)]),
      ...(workspace.worldbooks || []).flatMap((item) => [item.title, item.category, ...(item.rules || []).slice(0, 3)]),
    ...
    return [
      ...
      recent ? `Recent turns:\n${recent}` : "",
      workspaceText ? `Workspace hints: ${workspaceText}` : "",
    ]
      .filter(Boolean)
      .join("\n");
  }
