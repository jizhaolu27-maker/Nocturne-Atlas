...
function extractGroundingTerms(text) {
  return unique(
    extractSearchTerms(text, { maxItems: 60 })
...
function isAtmosphericClause(clause) {
  ...
  const descriptiveCue = includesCue(text, DESCRIPTIVE_CUES);
  const factualCue = includesCue(text, FACTUAL_CUES);
...
function getSupportFamilyStrength(termHits, matches = []) {
  return Math.max(Number(termHits) || 0, Number(matches?.[0]?.score) || 0);
}

function resolveDominantSupportType(scores = {}) {
  ...
  if (ranked.length > 1 && ranked[1][1] > 0 && ranked[0][1] - ranked[1][1] <= 1) {
    return "mixed";
  }
  return ranked[0][0];
}

function buildClauseSupportMetadata({
  supportRefs = [],
  knowledgeHits = 0,
  knowledgeMatches = [],
  memoryFactHits = 0,
  memoryFactMatches = [],
  memoryEvidenceHits = 0,
  memoryEvidenceMatches = [],
} = {}) {
  const supportScores = {
    knowledge: getSupportFamilyStrength(knowledgeHits, knowledgeMatches),
    memory_fact: getSupportFamilyStrength(memoryFactHits, memoryFactMatches),
    memory_evidence: getSupportFamilyStrength(memoryEvidenceHits, memoryEvidenceMatches),
  };
  return {
    supportType: resolveDominantSupportType(supportScores),
    primarySupportRef: supportRefs[0] || null,
    supportScores,
  };
}
...
function createGroundingCheckTools({ summarizeText } = {}) {
  function evaluateAssistantGrounding({
    ...
    for (const clause of clauses) {
      ...
      const supportMetadata = buildClauseSupportMetadata({
        supportRefs,
        knowledgeHits,
        knowledgeMatches,
        memoryFactHits,
      ...
      if (
        ...
        contestedClauses.push({
          ...
          supportType: supportMetadata.supportType,
          primarySupportRef: supportMetadata.primarySupportRef,
          contestedSupportType: "contested_memory",
          primaryContestedSupportRef: contestedSupportRefs[0] || null,
        });
        continue;
      }
