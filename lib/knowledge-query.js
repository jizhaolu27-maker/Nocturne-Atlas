...
function getWorkspaceEntityEntries(workspace = {}) {
  ...
  return entries.map((entry) => ({
    ...entry,
    terms: unique(entry.terms.flatMap((item) => extractSearchTerms(item, { maxItems: 16 }))),
  }));
}
...
function scoreFocusClause(clause, role, normalizedUserMessage, workspaceTerms) {
  ...
  const workspaceHits = workspaceTerms.filter((term) => clauseTerms.includes(term) || text.includes(term));
  ...
  if (/^(继续|揭示|记起|怎么|为什么|是谁|哪里|哪个|如何|讲讲)/.test(clause)) {
    score += 1;
  }
  return score;
}
