...
function createKnowledgeRetrievalTools({
  ...
  async function retrieveKnowledgeChunks({ story, workspace, userMessage = "", messages = [], embeddingOptions = {}, maxItems = 4 }) {
    ...
    const queryEmbeddingResult = await resolveEmbeddingResult(queryText, embeddingOptions);
    const queryEmbedding = Array.isArray(queryEmbeddingResult?.vector) ? queryEmbeddingResult.vector : null;
    ...
    const scored = await Promise.all(
      lexicalScored.map(async (item) => {
        ...
        const cachedEntry = shouldUseVector ? cacheEntries[cacheKey] || null : null;
        const cachedEmbedding = cachedEntry?.embedding || null;
        const itemEmbeddingResult =
          shouldUseVector && queryEmbedding && !(Array.isArray(cachedEmbedding) && cachedEmbedding.length)
        ...
        const itemEmbedding =
          shouldUseVector && Array.isArray(cachedEmbedding) && cachedEmbedding.length
            ? cachedEmbedding
            : shouldUseVector && queryEmbedding
        ...
        return {
          ...
          reasons: unique([
            ...
            vectorScore > 0.22 ? "Local vector similarity" : "",
          ]),
        };
      })
    );
    ...
    return {
      ...
      retrievalMeta: {
        ...
        vectorFailure: queryEmbedding ? "" : queryEmbeddingResult?.error || "",
