...
function buildKnowledgeFocusSignals(item, query, normalizedQuery) {
  ...
  const exactEntryHits = matchedEntries.filter((entry) => entry.terms.some((term) => term && (chunkTerms.has(term) || chunkText.includes(term))));
  const exactEntryBoost = Math.min(3.2, exactEntryHits.length * 1.6);

  let relationshipTargetHits = [];
  ...
  if (String(item?.sourceType || "").trim().toLowerCase() === "character" && String(item?.chunkType || "").trim().toLowerCase() === "relationships") {
    ...
    const matchedCharacterEntries = matchedEntries.filter((entry) => entry.sourceType === "character");
    const sourceFocusMatched = matchedCharacterEntries.some(
      (entry) => entry.id === normalizedSourceId || entry.terms.includes(normalizedTitle)
    );
    const counterpartTargets = matchedCharacterEntries.filter(
      (entry) => entry.id && entry.id !== normalizedSourceId && !entry.terms.includes(normalizedTitle)
    );
    relationshipTargetHits = counterpartTargets.filter((entry) =>
      entry.terms.some((term) => term && (chunkTerms.has(term) || chunkText.includes(term)))
    );
    if (relationshipTargetHits.length) {
      relationshipPrecisionPenalty = 0;
    } else if (sourceFocusMatched && counterpartTargets.length) {
      relationshipPrecisionPenalty = Math.min(6, 4 + counterpartTargets.length * 1.5);
    }
  }

  const relationshipPrecisionBoost = relationshipTargetHits.length * 5.5;
  const focusReasons = unique([
    exactEntryHits.length ? `Exact focus match: ${exactEntryHits.slice(0, 2).map((entry) => entry.label).filter(Boolean).join(", ")}` : "",
    relationshipTargetHits.length
      ? `Exact relationship target: ${relationshipTargetHits.slice(0, 2).map((entry) => entry.label).filter(Boolean).join(", ")}`
      : "",
  ]);
