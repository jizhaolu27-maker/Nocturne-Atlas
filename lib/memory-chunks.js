...
function createMemoryChunkTools({
  ...
  function buildEpisodicChunkMetadata(record = {}) {
    ...
    const entities = selectStableValues(record.entities, 4);
    const excludedTerms = new Set(
      [...subjectIds, ...objectIds]
        .map((value) => normalizeText(value))
        .filter(Boolean)
    );
    const tags = entities.filter((value) => !excludedTerms.has(normalizeText(value))).slice(0, 4);
    const hasStructuredAnchor = subjectIds.length > 0 || objectIds.length > 0 || entities.length > 0;
    const scope =
