...
const { MEMORY_SCHEMA_VERSION } = require("./memory-runtime");
...
function buildSourceMessageRange(records) {
  const ranges = (Array.isArray(records) ? records : [])
    .map((item) => (Array.isArray(item?.sourceMessageRange) ? item.sourceMessageRange : []))
    .filter((range) => range.length === 2)
    .map((range) => range.map((value) => Number(value)))
    .filter((range) => range.every(Number.isFinite));
  if (!ranges.length) {
    return [];
  }
  const starts = ranges.map((range) => Math.min(range[0], range[1]));
  const ends = ranges.map((range) => Math.max(range[0], range[1]));
...
function getConsolidationKey(record) {
  ...
  if (kind === "relationship_update" || kind === "world_state") {
    return buildMemoryStateSlot(record);
  }
  const canonKey = buildMemoryCanonKey(record);
  const facet = buildMemoryStateFacet(record);
  if (canonKey && facet && facet !== "generic") {
    return canonKey;
  }
  return buildMemoryStateSlot(record);
}
...
function consolidateMemoryRecords(memoryRecords, options = {}) {
  ...
  for (const [, group] of grouped.entries()) {
    ...
    const consolidated = {
      ...
      subjectIds,
      objectIds,
      sourceMessageRange: buildSourceMessageRange(group),
