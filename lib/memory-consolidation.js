...
function getConsolidationKey(record) {
  const kind = record.kind || "plot_checkpoint";
  const subjectIds = unique((record.subjectIds || []).map((item) => String(item || "").trim()).filter(Boolean)).sort();
  const objectIds = unique((record.objectIds || []).map((item) => String(item || "").trim()).filter(Boolean)).sort();

  if (kind === "character_update") {
    return `${kind}:${subjectIds[0] || "generic"}`;
  }
  if (kind === "world_state") {
    return `${kind}:${subjectIds[0] || "generic"}`;
  }
  if (kind === "relationship_update") {
    const relationshipIds = unique([...subjectIds, ...objectIds]).sort();
    return `${kind}:${relationshipIds.join("|") || "generic"}`;
  }
  return `${kind}:generic`;
}
...
function consolidateMemoryRecords(memoryRecords, options = {}) {
  ...
  for (const [, group] of grouped.entries()) {
    ...
    const consolidated = {
      ...
      tier: "long_term",
      kind,
      scope: group[0].scope || (kind === "world_state" ? "world" : kind === "relationship_update" ? "relationship" : "character"),
      summary,
