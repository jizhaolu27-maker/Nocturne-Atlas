...
function buildMemoryQuery({ userMessage = "", messages = [], workspace = {} }) {
  ...
  const continuation = /^(continue|go on|next|keep going|again|继续|接着|然后|后面|往下)/i.test(String(userMessage || "").trim());
  const inheritedEntities = continuation && primaryMatchedEntities.length === 0
    ? workspaceEntities.filter((entity) =>
        entity.terms.some((term) => term && normalizeText(recentText).includes(term))
      ).slice(0, 6)
    : [];
  const mergedMatchedEntities = unique([...matchedEntities, ...inheritedEntities].map((item) => item.id))
    .map((id) => workspaceEntities.find((item) => item.id === id))
    .filter(Boolean);
  return {
    ...
    matchedEntityIds: mergedMatchedEntities.map((item) => item.id).slice(0, 12),
    primaryMatchedEntityIds: primaryMatchedEntities.map((item) => item.id).slice(0, 8),
    matchedEntityScopes: unique(mergedMatchedEntities.map((item) => item.scope)).slice(0, 6),
    inheritedEntityIds: inheritedEntities.map((item) => item.id),
    ...
    embeddingText: [
      ...
      mergedMatchedEntities.length
        ? `Entity focus: ${mergedMatchedEntities
            .slice(0, 6)
