...
function refreshEmbeddingSignature(item = {}) {
  ...
  const nextSignature = buildEmbeddingSignature(item.embeddingProvider, item.embeddingModel);
  const changed = String(item.embeddingSignature || "") !== nextSignature;
  return {
    item: changed
      ? {
          ...item,
...
function normalizeStoredMemoryRecord(record = {}) {
  ...
  const keywordRefresh = refreshKeywords(record, summary, 12);
  const schemaItem = keywordRefresh.item.schemaVersion === MEMORY_SCHEMA_VERSION
    ? keywordRefresh.item
    : { ...keywordRefresh.item, schemaVersion: MEMORY_SCHEMA_VERSION };
  const signatureRefresh = refreshEmbeddingSignature(schemaItem);
  return {
    item: signatureRefresh.item,
    changed: keywordRefresh.changed || signatureRefresh.changed || schemaItem !== keywordRefresh.item,
  };
}
...
function normalizeStoredMemoryChunk(chunk = {}) {
  ...
  const keywordRefresh = refreshKeywords(chunk, text, 12);
  const schemaItem = keywordRefresh.item.schemaVersion === MEMORY_SCHEMA_VERSION
    ? keywordRefresh.item
    : { ...keywordRefresh.item, schemaVersion: MEMORY_SCHEMA_VERSION };
  const signatureRefresh = refreshEmbeddingSignature(schemaItem);
