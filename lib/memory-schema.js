...
function extractFallbackTerms(text) {
  return extractSearchTerms(text, { maxItems: 16 }).map(normalizeText);
}
...
function buildMemoryStateSlot(record = {}) {
  if (String(record.stateSlot || "").trim()) {
    return normalizeText(record.stateSlot);
  }
  ...
  if (kind === "relationship_update") {
    ...
    return `relationship:${participants.join("|") || getRecordAnchor(record)}`;
  }
  if (kind === "character_update") {
    return `character:${getNormalizedIds(record.subjectIds, 1)[0] || getRecordAnchor(record)}`;
  }
  if (kind === "world_state") {
    return `world:${
      getNormalizedIds(record.subjectIds, 1)[0] ||
      getNormalizedIds(record.objectIds, 1)[0] ||
      getRecordAnchor(record)
    }`;
  }
  return `${getRecordScope(record)}:${getRecordAnchor(record)}`;
}

function buildMemoryStateFacet(record = {}) {
  if (String(record.stateFacet || "").trim()) {
    return normalizeText(record.stateFacet);
  }
  const anchor = getRecordAnchor(record);
  const detailTerms = getRecordFocusTerms(record, 6)
    .filter((term) => term && term !== anchor)
    .slice(0, 2);
  return detailTerms.join("|") || "generic";
}
...
function buildMemoryCanonKey(record = {}) {
  ...
  return `${buildMemoryStateSlot(record)}:${buildMemoryStateFacet(record)}`;
}
