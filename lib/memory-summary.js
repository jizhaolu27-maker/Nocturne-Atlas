...
const { MEMORY_KEYWORD_VERSION, MEMORY_SCHEMA_VERSION } = require("./memory-runtime");
const { buildMemoryCanonKey, buildMemoryConflictGroup, buildMemoryStateFacet, buildMemoryStateSlot } = require("./memory-schema");
const {
  normalizeLongText,
  normalizeStringList,
  normalizeText,
...
function createMemorySummaryTools({
  ...
  async function applyMemoryRecordEmbedding(record, story) {
    ...
    const embeddingResult =
      typeof embedTextDetailed === "function"
        ? await embedTextDetailed(buildMemoryEmbeddingText(target), embeddingOptions)
        : {
            vector: await embedText(buildMemoryEmbeddingText(target), embeddingOptions),
  ...
  async function createStructuredMemoryRecord({
    ...
    record.stateSlot = buildMemoryStateSlot(record);
    record.stateFacet = buildMemoryStateFacet(record);
    record.conflictGroup = buildMemoryConflictGroup(record);
    record.canonKey = buildMemoryCanonKey(record);
    return applyMemoryRecordEmbedding(record, story);
  }
