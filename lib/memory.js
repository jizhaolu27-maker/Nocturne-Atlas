...
function createMemoryTools({
  ...
  function deleteMemoryRecord(storyId, recordId) {
    const id = String(recordId || "").trim();
    if (!id) throw new Error("Memory id is required");
    const records = readJsonLines(getStoryMemoryFile(storyId));
    const nextRecords = records.filter((item) => item.id !== id);
    if (nextRecords.length === records.length) throw new Error("Memory record not found");
    const chunks = readJsonLines(getStoryMemoryChunkFile(storyId));
    const removedChunkCount = chunks.filter((item) => item.linkedRecordId === id).length;
    writeJsonLines(getStoryMemoryFile(storyId), nextRecords);
    writeJsonLines(getStoryMemoryChunkFile(storyId), chunks.filter((item) => item.linkedRecordId !== id));
    return { id, removedChunkCount };
  }
