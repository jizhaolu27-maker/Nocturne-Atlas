...
function createProposalTools({
  ...
  function getProposalDedupKey(item) {
    ...
    if (targetType === "story_state") {
      ...
      if (patch.kind === "relationship") {
        return `update:${targetType}:relationship:${String(mapItem.sourceId || "").toLowerCase()}>${String(mapItem.targetId || "").toLowerCase()}:${String(mapItem.type || "").toLowerCase()}`;
      }
  ...
  function proposalComparableText(item) {
    const patch = item?.patch || item?.diff || {};
    const mapItem = patch.item || {};
    return [mapItem.title, mapItem.label, mapItem.summary, mapItem.note, patch.title, patch.storyState, item?.reason]
      .filter(Boolean).join(" ").toLowerCase().replace(/[^\p{L}\p{N}]+/gu, "");
  }

  function isSimilarProposal(left, right) {
    if (!left || !right || left.targetType !== right.targetType) return false;
    if (left.targetId && right.targetId && left.targetId === right.targetId && left.targetType !== "story_state") return true;
    if (left.targetType !== "story_state" || left.patch?.kind !== right.patch?.kind) return false;
    if (left.patch?.kind === "relationship") {
      return left.patch?.item?.sourceId === right.patch?.item?.sourceId &&
        left.patch?.item?.targetId === right.patch?.item?.targetId &&
        left.patch?.item?.type === right.patch?.item?.type;
    }
    const a = proposalComparableText(left);
    const b = proposalComparableText(right);
  ...
  async function tryGenerateProposals(story, messages, workspace, signal, storyState = null) {
    ...
    const prompt = [
      {
        role: "system",
        content:
          "Review recent fictional story messages and the supplied current files. Return JSON: { proposals: [{ action, targetType, targetId, reason, patch }] }. Make one call that both decides whether a proposal is needed and drafts it; return an empty proposals array when nothing durable changed. You may return multiple proposals only when they represent separate, non-overlapping changes, maximum 5. Compare every proposed update with the supplied original file or Map entry first; prefer one update over duplicate creates and do not restate an existing fact. targetType may be character, worldbook, style, or story_state. For story_state, patch must be { kind: outline|thread|timeline|relationship, item: {...} } and only represent a meaningful reviewed story change. A relationship item has this shape: { id, sourceId, targetId, type, label, direction: \"directed\", strength, status, storyTime, sortKey, outlineNodeId, timelineEventId, note }; sourceId and targetId must be two different ids from workspace.characters. Every relationship arrow represents only sourceId's current attitude toward targetId. label must be that one-sided attitude, such as 感兴趣, 在意, 难以描述的存在, 光, or 爱恨交织, never a shared or reciprocal relationship phrase. If both characters' attitudes change, return two relationship proposals with reversed sourceId and targetId and independently accurate labels; never collapse them into direction=mutual. A timeline item has this shape: { id, title, summary, status, storyTime, sortKey, outlineNodeId, plotThreadIds, characterIds, locationId, sourceMessageRange }; title or summary is required. Use timeline for events that happened, thread/outline for current direction and goals, relationship for durable relationship changes. Use worldbook only for durable world rules, organizations, locations, stable setting facts, or a changed present condition of an existing worldbook subject; do not put ordinary plot events, scene details, character arcs, future goals, or temporary state there. A worldbook storyState is only a concise present-condition summary. action must be update or create; story_state uses update. Only allow create for targetType=character when the story clearly introduces a meaningful new recurring character. Do not create one-off extras. Keep every field compact and retrieval-friendly, not literary. reason must be one short sentence, ideally 20-60 Chinese characters or under 18 English words. For create character patches, include name, core, traits, relationships, arcState, and notes, but keep them terse.",
      },
      {
        role: "user",
        content: JSON.stringify(
    ...
    try {
      ...
      const seen = new Set();
      const characterIds = new Set((workspace.characters || []).map((item) => item.id));
      return parsed.proposals
        ...
        .filter((item) => {
          if (item.targetType !== "story_state" || item.patch?.kind !== "relationship") return true;
          const relation = item.patch?.item || {};
          return Boolean(relation.sourceId && relation.targetId && relation.sourceId !== relation.targetId && relation.direction === "directed" && String(relation.label || "").trim() && characterIds.has(relation.sourceId) && characterIds.has(relation.targetId));
        })
        .filter((item) => {
  ...
  async function generateManualProposals({ storyId, signal }) {
    ...
    for (const item of generated) {
      if (existingKeys.has(getProposalDedupKey(item)) || proposals.some((candidate) => isSimilarProposal(candidate, item))) continue;
      proposals.push(item);
    }
