...
function createProviderTools({
  ...
  function getAppSecretFile() {
    return path.join(CONFIG_DIR, "app-secret.json");
  }

  function getLegacyAppSecretFile() {
    return `${CONFIG_DIR}\\app-secret.json`;
  }

  function getAppSecret() {
    const filePath = getAppSecretFile();
    const current = readJson(filePath);
    if (current?.secret) {
      return current.secret;
    }

    // Older releases used a literal backslash. On POSIX that created a
    // sibling file named "config\\app-secret.json" instead of a file inside
    // the config directory. Import it so existing provider keys still work.
    const legacyFilePath = getLegacyAppSecretFile();
    if (legacyFilePath !== filePath && fs.existsSync(legacyFilePath)) {
      const legacy = readJson(legacyFilePath);
      if (legacy?.secret) {
  ...
  async function sendChatCompletionsRequest(url, headers, payload, { allowCompatFallback = true, signal } = {}) {
