...
function extractFusionTerms(text) {
  return unique(
    (String(text || "").match(/[A-Za-z][A-Za-z0-9_-]{2,}|[\u4e00-\u9fff]{2,6}/g) || [])
...
function buildFamilyBudgets(route, totalBudget, counts) {
  ...
  if (remaining > 0 && counts.knowledge > knowledgeBudget) {
    const addKnowledge = Math.min(remaining, counts.knowledge - knowledgeBudget);
    knowledgeBudget += addKnowledge;
    remaining -= addKnowledge;
  }
  if (remaining > 0 && counts.memory > memoryBudget) {
    const addMemory = Math.min(remaining, counts.memory - memoryBudget);
    memoryBudget += addMemory;
    remaining -= addMemory;
  }
  ...
  if (memoryRemaining > 0 && counts.memoryFacts > factBudget) {
    const addFacts = Math.min(memoryRemaining, counts.memoryFacts - factBudget);
    factBudget += addFacts;
    memoryRemaining -= addFacts;
  }
  if (memoryRemaining > 0 && counts.memoryEvidence > evidenceBudget) {
    const addEvidence = Math.min(memoryRemaining, counts.memoryEvidence - evidenceBudget);
    evidenceBudget += addEvidence;
  }
...
function computeReasonBoost(reasons = []) {
  ...
  let boost = 0;
  if (rows.some((item) => item.includes("entity match"))) {
    boost += 1.2;
  }
  if (rows.some((item) => item.includes("keyword match"))) {
    boost += 0.7;
  }
