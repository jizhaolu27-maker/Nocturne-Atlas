...
function createServerConfigTools({
  ...
  function normalizeMemoryRetrievalMode(value, fallback = "lexical") {
    const normalized = String(value || "").trim().toLowerCase();
    if (normalized === "hybrid" || normalized === "rag") {
      return normalized;
    }
    return fallback;
  }

  function normalizeKnowledgeRetrievalMode(value, fallback = "lexical") {
    return String(value || "").trim().toLowerCase() === "hybrid" ? "hybrid" : fallback;
  }
  ...
  function normalizeAppConfig(config = {}) {
    const memoryRetrievalMode = normalizeMemoryRetrievalMode(config.memoryRetrievalMode, "lexical");
    const localEmbedding = normalizeEmbeddingConfig(config.localEmbedding || {});
    const hasExplicitKnowledgeRetrievalMode =
      String(config.knowledgeRetrievalMode || "").trim().toLowerCase() === "lexical" ||
      String(config.knowledgeRetrievalMode || "").trim().toLowerCase() === "hybrid";
    const knowledgeRetrievalMode = hasExplicitKnowledgeRetrievalMode
      ? normalizeKnowledgeRetrievalMode(config.knowledgeRetrievalMode, "lexical")
      : localEmbedding.mode === "on"
        ? "hybrid"
        : "lexical";
    return {
      ...
      knowledgeRetrievalModeSource: hasExplicitKnowledgeRetrievalMode ? "explicit" : "derived",
      localEmbedding,
    };
  }
  ...
  function buildNextStorySettings(story, settings = {}) {
    ...
    if (["inherit", "lexical", "hybrid", "rag"].includes(String(settings.memoryRetrievalMode || ""))) {
      next.memoryRetrievalMode = String(settings.memoryRetrievalMode);
    }
    if (["inherit", "lexical", "hybrid"].includes(String(settings.knowledgeRetrievalMode || ""))) {
      next.knowledgeRetrievalMode = String(settings.knowledgeRetrievalMode);
    }
