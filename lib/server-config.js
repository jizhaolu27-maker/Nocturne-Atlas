...
function createServerConfigTools({
  ...
  isTransformersDependencyInstalled = () => {
    try {
  ...
  function normalizeInjectionModes(value, current = {}) {
    const next = { ...(current || {}) };
    for (const type of ["characters", "worldbooks", "styles"]) {
      const source = value?.[type] && typeof value[type] === "object" ? value[type] : {};
      const currentType = current?.[type] && typeof current[type] === "object" ? current[type] : {};
      next[type] = { ...currentType };
      for (const [id, mode] of Object.entries(source)) {
        if (isSafeStorageId(id) && ["always", "keyword"].includes(String(mode))) {
          next[type][id] = String(mode);
        }
      }
    }
    return next;
  ...
  function buildNextStorySettings(story, settings = {}) {
    ...
    delete next.memoryRetrievalMode;
    delete next.knowledgeRetrievalMode;
    delete next.localEmbeddingMode;
    return next;
  }
  ...
  function buildNextStoryEnabled(story, enabled = {}) {
    ...
    const next = {
      ...
      injectionModes: normalizeInjectionModes({}, current.injectionModes),
    };
    ...
    if (Object.prototype.hasOwnProperty.call(enabled, "injectionModes")) {
      next.injectionModes = normalizeInjectionModes(enabled.injectionModes, current.injectionModes);
    }
    return next;
  }
  ...
  function getLocalEmbeddingRuntimeStatus(config = {}) {
    ...
    const dependencyInstalled = Boolean(isTransformersDependencyInstalled());
  ...
  function resolveStoryEmbeddingConfig(story) {
    ...
    return normalizeEmbeddingConfig({
      ...
      mode: normalizeEmbeddingMode(appConfig.localEmbedding?.mode || "off", "off"),
    });
  }
