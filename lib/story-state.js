...
function createStoryStateTools({ readJson, writeJson, getStoryStateFile, getStory, safeId }) {
  ...
  function normalizeTimelineEvent(item = {}, index = 0) {
    const hasMeaningfulContent = [
      ...
      item.outlineNodeId,
      item.locationId,
      ...(Array.isArray(item.plotThreadIds) ? item.plotThreadIds : []),
      ...(Array.isArray(item.characterIds) ? item.characterIds : []),
      ...(Array.isArray(item.sourceMessageRange) ? item.sourceMessageRange : []),
    ].some((value) => String(value || "").trim());
    // Ignore malformed empty events left by older model proposals instead of
    // turning them into visible "Untitled event" entries during migration.
  ...
  function normalizeStoryState(value = {}, options = {}) {
    ...
    const state = {
      ...
      timelineEvents: (Array.isArray(source.timelineEvents) ? source.timelineEvents : []).slice(0, 2000).map(normalizeTimelineEvent).filter(Boolean),
