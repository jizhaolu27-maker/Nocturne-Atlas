...
function createStoryStore({
  ...
  function ensureFile(filePath, fallback) {
    ...
    try {
      fs.writeFileSync(filePath, fallback, { encoding: "utf8", flag: "wx" });
    } catch (error) {
    ...
    if (fs.existsSync(filePath)) {
      ...
      if (size > 0) {
        ...
        try {
          ...
          fs.readSync(file, lastByte, 0, 1, size - 1);
          separator = lastByte[0] === 10 ? "" : "\n";
        } finally {
          fs.closeSync(file);
        }
      }
    }
    fs.appendFileSync(filePath, `${separator}${JSON.stringify(value)}\n`, "utf8");
  }
  ...
  function getStoryMemoryArchiveFile(storyId) {
    return path.join(getStoryDir(storyId), "memory", "archive.jsonl");
  }

  function getStoryKnowledgeChunkFile(storyId) {
    return path.join(getStoryDir(storyId), "knowledge", "chunks.jsonl");
  }
  ...
  function getStoryStateFile(storyId) {
    return path.join(getStoryDir(storyId), "story-state", "state.json");
  }
  ...
  function createDefaultStory(payload = {}) {
    ...
    const story = {
      ...
      enabled: {
        ...
        injectionModes: payload.enabled?.injectionModes || { characters: {}, worldbooks: {}, styles: {} },
      },
      contextStatus: {
        usedTokens: 0,
        maxTokens: 32000,
    ...
    ensureDir(path.join(getStoryDir(storyId), "story-state"));
    ...
    ensureFile(getStoryMemoryArchiveFile(storyId), "");
    ensureFile(getStoryKnowledgeChunkFile(storyId), "");
    ...
    ensureFile(
      getStoryStateFile(storyId),
      JSON.stringify({ schemaVersion: 1, outlineNodes: [], plotThreads: [], timelineEvents: [], relationshipEvents: [], updatedAt: now }, null, 2)
    );
