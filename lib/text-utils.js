...
const QUESTION_LIKE_PREFIX_PATTERN =
  /^(?:continue|what|how|why|who|where|when|remember|reveal|show|tell|which|继续|怎么|为什么|是谁|哪里|什么时候|记得|揭示|展示|告诉|哪个)/i;
...
const WEAK_CJK_BIGRAM_CHARS = new Set(
  [
    "\u7684",
    "\u4e86",
    "\u5728",
...
function isQuestionLikeClause(text) {
  const value = String(text || "").trim();
  if (!value) {
    return false;
  }
  return QUESTION_MARK_PATTERN.test(value) || QUESTION_LIKE_PREFIX_PATTERN.test(value);
}
...
function hasWeakTwoCharCjkPattern(term) {
  if (!isPureCjkTerm(term) || term.length !== 2) {
    return false;
  }
  if (CJK_WEAK_TERMS.has(term)) {
    return true;
  }
  const [first, second] = Array.from(term);
  if (WEAK_CJK_BIGRAM_CHARS.has(first) || WEAK_CJK_BIGRAM_CHARS.has(second)) {
    return true;
  }
  if (first === "\u5c0f" && /[\u59d0\u591c\u706f\u8def\u5df7\u5e97\u5c4b]/.test(second)) {
    return true;
  }
  return false;
}

function hasWeakThreeCharCjkPattern(term) {
  if (!isPureCjkTerm(term) || term.length !== 3) {
    return false;
  }
  const [first, second, third] = Array.from(term);
  if (WEAK_CJK_BIGRAM_CHARS.has(first) || WEAK_CJK_BIGRAM_CHARS.has(third)) {
    return true;
  }
  if (WEAK_CJK_BIGRAM_CHARS.has(second)) {
    return true;
  }
  if (third === "\u53bb" || third === "\u6765" || third === "\u4e86" || third === "\u7740" || third === "\u8fc7") {
    return true;
  }
  return false;
}
...
function isMeaningfulSearchTerm(value) {
  ...
  if (isPureCjkTerm(term)) {
    ...
    if (hasWeakTwoCharCjkPattern(term)) {
      return false;
    }
    if (hasWeakThreeCharCjkPattern(term)) {
      return false;
    }
