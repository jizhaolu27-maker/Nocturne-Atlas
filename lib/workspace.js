...
function createWorkspaceTools({
  ...
  function loadWorkspaceItems(storyId, type) {
    return listJsonFiles(getStoryWorkspaceDir(storyId, type)).map((item) => {
      if (!Object.prototype.hasOwnProperty.call(item, "changeLog")) {
        return item;
      }
      const { changeLog, ...clean } = item;
      writeJson(path.join(getStoryWorkspaceDir(storyId, type), `${item.id}.json`), clean);
      return clean;
    });
  }
