...
function getWorkspaceTerms(workspace) {
  const items = [
    ...(workspace.characters || []).flatMap((item) => [item.id, item.name, item.core?.role, ...(item.traits || [])]),
    ...(workspace.worldbooks || []).flatMap((item) => [item.id, item.title, item.category, ...(item.rules || [])]),
